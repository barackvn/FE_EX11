    ...
    def _onchange_sh_invoice_barcode_mobile(self):
        ...
        if not self.partner_id:
            if self.env.user.company_id.sudo().sh_invoice_bm_is_notify_on_fail:
                message = _(f'{CODE_SOUND_FAIL}You must first select a partner!')
            ...
            if self.state != "draft":
                ...
                if self.env.user.company_id.sudo().sh_invoice_barcode_mobile_type == "barcode":            
                    ...
                    if lote := self.env["stock.production.lot"].search(
                        [('name', '=', self.sh_invoice_barcode_mobile)]
                    ):
                ...
                if search_lines:
                    ...
                    if search_product.uom_id:
                        invoice_line_val["uom_id"] = search_product.uom_id.id    
                ...
                elif self.env.user.company_id.sudo().sh_invoice_bm_is_notify_on_fail:    
                    message = _(
