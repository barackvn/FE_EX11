...
class purchase_order(models.Model):
    ...
    def _onchange_sh_purchase_barcode_mobile(self):
        ...
        if self:
            if (
                self.state in ["cancel", "done"]
                and self.sh_purchase_barcode_mobile
            ):
            ...
            elif self.sh_purchase_barcode_mobile:
                search_lines = False
                domain = []
                if self.env.user.company_id.sudo().sh_purchase_barcode_mobile_type == "barcode":            
                    ...
                    if lote := self.env["stock.production.lot"].search(
                        [('name', '=', self.sh_purchase_barcode_mobile)]
                    ):
                ...
                if search_lines:
                    ...
                    if search_product.uom_id:
                        order_line_val["product_uom"] = search_product.uom_po_id.id    
                    ...
                    if self.env.user.company_id.sudo().sh_purchase_bm_is_notify_on_success:
                        message = _(f'{CODE_SOUND_SUCCESS}Product: %s Qty: %s') % (
                            ...
                            new_order_line.product_qty,
                        )
                        self.env.user.notify_info(message, title=_('Succeed'), sticky=False)   


                elif self.env.user.company_id.sudo().sh_purchase_bm_is_notify_on_fail:    
                    message = _(
